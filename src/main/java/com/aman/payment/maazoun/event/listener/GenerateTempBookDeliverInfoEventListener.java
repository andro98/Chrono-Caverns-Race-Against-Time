...
@Component
public class GenerateTempBookDeliverInfoEventListener implements ApplicationListener<GenerateTempBookDeliverInfoEvent> {
	...
	@Override
	public synchronized void onApplicationEvent(GenerateTempBookDeliverInfoEvent event) {
		
		List<MaazounDeliverInfoReportDTO> collection = (List<MaazounDeliverInfoReportDTO>) (event.getSource());
		
		InputStream receiptStream = this.getClass().getResourceAsStream(ConfigurationConstant.MAAZOUN_DELIVER_REQUEST_FORM);
		...
		try {
			...
			if(event.getRegenerateFlag() == 1) {
				JasperExportManager.exportReportToPdfFile(receiptPrint, event.getReceiptUrl());
			}else {
