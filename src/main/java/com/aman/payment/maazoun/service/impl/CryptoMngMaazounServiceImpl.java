...
/**
 * @author Assem Abdelaziz
 */

@Service
public class CryptoMngMaazounServiceImpl implements CryptoMngrMaazounService {
	...
	private byte[] encrypt(final byte[] key, final byte[] IV, final byte[] message) throws Exception {
		return CryptoMngMaazounServiceImpl.encryptDecrypt(Cipher.ENCRYPT_MODE, key, IV, message);
	}
	...
	private byte[] decrypt(byte[] key, byte[] iv, byte[] message) throws Exception {
		return CryptoMngMaazounServiceImpl.encryptDecrypt(Cipher.DECRYPT_MODE, key, iv, message);
	}
