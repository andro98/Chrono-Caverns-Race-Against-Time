...
@Service
public class CryptoMngrCoreServiceImpl implements CryptoMngrCoreService {
	...
	private byte[] encrypt(final byte[] key, final byte[] IV, final byte[] message) throws Exception {
		return CryptoMngrCoreServiceImpl.encryptDecrypt(Cipher.ENCRYPT_MODE, key, IV, message);
	}
	...
	private byte[] decrypt(byte[] key, byte[] iv, byte[] message) throws Exception {
		return CryptoMngrCoreServiceImpl.encryptDecrypt(Cipher.DECRYPT_MODE, key, iv, message);
	}
