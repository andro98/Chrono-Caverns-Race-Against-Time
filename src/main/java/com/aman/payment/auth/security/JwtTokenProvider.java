...
public class JwtTokenProvider {
    ...
    public String generateToken(CustomUserDetails customUserDetails) {
        ...
        return Jwts.builder()
                .setSubject(String.valueOf(customUserDetails.getId()))//cryptoMngrAuthService.encrypt(String.valueOf(customUserDetails.getId())))
    ...
    public String generateTokenFromUserId(Long userId) {
        Instant expiryDate = Instant.now().plusMillis(jwtExpirationInMs);
        return Jwts.builder()
                .setSubject(String.valueOf(userId))//cryptoMngrAuthService.encrypt(String.valueOf(userId)))
                .setIssuedAt(Date.from(Instant.now()))
                .setExpiration(Date.from(expiryDate))
                .signWith(SignatureAlgorithm.HS512, jwtSecret)
                .compact();
