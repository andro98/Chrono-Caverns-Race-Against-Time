...
import com.aman.payment.auth.model.lookup.Mail;
...
public class MailService {
    ...
    public void sendEmailVerification(String emailVerificationUrl, User user, String password)
            throws IOException, TemplateException, MessagingException {
        Mail mail = new Mail();
        mail.setSubject("Email Verification [AMAN Team Support]");
        mail.setTo(user.getEmail());
        mail.setFrom(mailFrom);
        mail.getModel().put("user", user.getFirstName()+" "+user.getLastName());
     ...
     */
    public void sendResetLink(String resetPasswordLink, User user)
            throws IOException, TemplateException, MessagingException {
        ...
        Mail mail = new Mail();
        mail.setSubject("Password Reset Link [AMAN Team Support]]");
        mail.setTo(user.getEmail());
        mail.setFrom(mailFrom);
        mail.getModel().put("userName", user.getUsername());
    ...
    public void sendAccountChangePassword(String action, String actionStatus, User user, String password)
            throws IOException, TemplateException, MessagingException {
        Mail mail = new Mail();
        mail.setSubject("Account Password Change [AMAN Team Support]");
        mail.setTo(user.getEmail());
        mail.setFrom(mailFrom);
        mail.getModel().put("userName", user.getUsername());
        mail.getModel().put("action", action);
        ...
        templateConfiguration.setClassForTemplateLoading(getClass(), basePackagePath);
        Template template = templateConfiguration.getTemplate("account-activity-change-password.ftl");
        String mailContent = FreeMarkerTemplateUtils.processTemplateIntoString(template, mail.getModel());
        mail.setContent(mailContent);
        send(mail);
    }
